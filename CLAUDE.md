...
4. UFLDv2 estimates the ego-lane corridor on the configured `lane_every` interval; cached/smoothed geometry is used between runs. The last committed detection is Kalman-coasted through brief misses; only after a longer miss streak (~10 lane-detection frames) does the corridor fall back to the fixed default, so faint-marking dropouts do not flash the lane away every few frames.
...
9. Road/lane relevance filtering removes detections that should not enter the risk tracker. Opposite-lane (oncoming) traffic — a detection firmly on the far side (`lane_position ≤ -1.35`) that does not overlap the ego corridor — is rejected up front, before the confidence/near cut-in escapes, so oncoming vehicles never get a track/box even when lane geometry is untrusted (the default corridor). A genuine lead that merely clamps to `pos = -1.5` still overlaps the corridor and is kept.
